...
//     private static double EARTH_RADIUS = 6357000.0; //源码中使用半径为6367460.0;
//     private static double EARTH_RADIUS = 6378000.0; //源码中使用半径为6367460.0;


    public static double calculateAreaOfGPSPolygonOnEarthInSquareMeters(final List<Point> locations) {
        return calculateAreaOfGPSPolygonOnSphereInSquareMeters(locations, EARTH_RADIUS);
    }

    private static double calculateAreaOfGPSPolygonOnSphereInSquareMeters(final List<Point> locations, final double radius) {
        if (locations.size() < 3) {
            return 0;
        }
...
//            LogUtils.e(String.format("Y %s: %s", listY.size() - 1, listY.get(listY.size() - 1)));
            listX.add(calculateXSegment(longitudeRef, longitude, latitude, circumference));
//            LogUtils.e(String.format("X %s: %s", listX.size() - 1, listX.get(listX.size() - 1)));
        }
        ...
        for (int i = 1; i < listX.size(); i++) {
            ...
            LogUtils.d( String.format("area %s: %s", listArea.size() - 1, listArea.get(listArea.size() - 1)));
        }
