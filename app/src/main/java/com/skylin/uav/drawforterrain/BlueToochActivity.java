...
public class BlueToochActivity extends BaseActivity {

    public static String Id = null;        // AotActivity 数据保存  ID
    public static String insertTime = null;    // AotActivity 数据保存  插入数据库时间
    public static String title = null;   // AotActivity 数据保存  任务名
    public static ArrayList<Point> boundaryList;            // AotActivity 数据保存  边界点集合
    public static ArrayList<ObsMasterBan> obsList;          // AotActivity 数据保存  障碍集合
    public static ArrayList<Pair<String, Point>> backout_list;  // AotActivity 数据保存  障碍集合

    public static final String RECOVER = "recover";   // AotActivity 是否保存了数据
    public static final String DETIAL = "detial";   // AotActivity 是否是详情
    public static final String INFOBEAN = "infobean";   //AotActivity数据
...
//        }
    ...
    public static void select() {
        if (!mConnected) {
            if (mBluetoothLeService == null | device == null) return;
            mBluetoothLeService.connect(device.getAddress());
        }
    }
    ...
    private final BroadcastReceiver mGattUpdateReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            if (BluetoothLeService.ACTION_GATT_CONNECTED.equals(intent.getAction())) { //连接一个GATT服务
                ...
                mConnected = false;
                ToastUtil.show(getString(R.string.BlueBase_tv3));
                channle = -1;  //设备信道
                workmoder = "";  //设备工作模式
    ...
    public void displayGattServices(final List<BluetoothGattService> gattServices) {
        ...
        for (int i = 0; i < gattServices.size(); i++) {
            ...
            for (int j = 0; j < characteristics.size(); j++) {
                ...
                if (C_uuid.startsWith("0000fff6")) {
//                if (C_uuid.startsWith("00001111")  ) {
    ...
    public static void writeBlue(byte[] bytes) {
        ...
        if (BlueToochActivity.mCharacteristic == null) return;
        try {
...
//                byte[] bytes = s.getBytes();
                if (bytes.length > 20) {
                    for (int k = 0; k < bytes.length; k += 20) {
                        ...
                        BlueToochActivity.mBluetoothLeService.writeCharacteristic(mCharacteristic);
                        Thread.sleep(10);
                    }
                } else {
