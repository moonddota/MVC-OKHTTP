...
package com.skylin.mavlink.connection.usb.usbserial.driver;
...
import com.skylin.mavlink.connection.usb.usbserial.util.HexDump;
...
public class FtdiSerialDriver extends CommonUsbSerialDriver {
    ...
    public void reset() throws IOException {
        ...
        if (result != 0) {
            throw new IOException("Reset retry: result=" + result);
        }
    ...
    private int setBaudRate(int baudRate) throws IOException {
        ...
        if (result != 0) {
            throw new IOException("Setting baudrate retry: result=" + result);
        }
    ...
    public void setParameters(int baudRate, int dataBits, int stopBits, int parity)
        ...
        int result = mConnection.controlTransfer(FTDI_DEVICE_OUT_REQTYPE,
                ...
                null, 0, USB_WRITE_TIMEOUT_MILLIS);
        if (result != 0) {
            throw new IOException("Setting parameters retry: result=" + result);
        }
    }
